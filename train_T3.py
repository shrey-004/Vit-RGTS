import vit_rgts.main_T3
print("✅ Loaded VitRGTS from:", vit_rgts.main_T3.__file__)

from einops import rearrange
from vit_rgts.main_T3 import pair
...
from vit_rgts.main_T3 import VitRGTS
...
def extract_tokens_patches_labels(model, loader, device, patch_size):
    """
    Returns:
      tokens: (N_total_patches, dim)  Tensor
      patches: (N_total_patches, C, pH, pW) Tensor normalized in [0,1]
      labels: (N_total_patches,) long tensor (parent image class)
    """
    model.eval()
    all_tokens = []
    all_patches = []
    all_labels = []
    with torch.no_grad():
        for imgs, labels in tqdm(loader, desc="Extracting tokens+patches"):
            imgs = imgs.to(device)
            b = imgs.shape[0]

            # forward to get tokens
            ...
            # extract raw patches same as to_patch_embedding Rearrange uses:
            # We need the raw image patches as tensors (C, pH, pW)
            # We'll use einops rearrange to get patches
            # imgs shape (b, C, H, W), patch_size = (pH,pW) or scalar
            pH, pW = pair(patch_size)
            patches = rearrange(imgs, 'b c (h p1) (w p2) -> b (h w) c p1 p2', p1=pH, p2=pW)
            # patches shape (b, n_patches, C, pH, pW)
            # move to CPU for storage
            all_tokens.append(patch_tokens.cpu())
            all_patches.append(patches.cpu())
            # replicate labels per patch:
            all_labels.append(labels.unsqueeze(1).expand(-1, n_patches).reshape(-1).cpu())

    tokens = torch.cat(all_tokens, dim=0)   # (num_images, n_patches, dim)
    patches = torch.cat(all_patches, dim=0) # (num_images, n_patches, C, pH, pW)
    labels = torch.cat(all_labels, dim=0)   # (num_images*n_patches,)

    B, N, D = tokens.shape
    tokens = tokens.view(B * N, D)
    patches = patches.view(B * N, *patches.shape[2:])  # (B*N, C, pH, pW)

    return tokens.float(), patches.float(), labels.long()




class PatchDecoder(nn.Module):
    """Simple MLP decoder that maps token -> flattened patch pixels."""
    def __init__(self, in_dim, out_dim, hidden=1024):
        ...
        self.net = nn.Sequential(
            ...
            nn.Sigmoid()  # assuming patches normalized to [0,1]
        )
    def forward(self, x):
        return self.net(x)

class PatchClassifier(nn.Module):
    """Linear probe / small MLP to classify parent image class from a token."""
...
def train_decoder(X_tr, Y_tr_patches, X_va, Y_va_patches, in_dim, out_dim, device, epochs=10, bs=2048, lr=1e-3):
    model = PatchDecoder(in_dim, out_dim).to(device)
    opt = torch.optim.Adam(model.parameters(), lr=lr)
    crit = nn.MSELoss()

    tr_ds = torch.utils.data.TensorDataset(X_tr, Y_tr_patches)
    va_ds = torch.utils.data.TensorDataset(X_va, Y_va_patches)
    ...
    with torch.no_grad():
        ...
        for xb, yb in va_loader:
            xb, yb = xb.to(device), yb.to(device)
            pred = model(xb)
            mse_list.append(((pred - yb)**2).mean(dim=1).cpu())
        mse = torch.cat(mse_list).mean().item()
        # PSNR approx: 10 * log10(1 / MSE) (patches in [0,1]) — handle zero
        psnr = 10.0 * math.log10(1.0 / (mse + 1e-12))
    return model, mse, psnr

def train_classifier(X_tr, y_tr, X_va, y_va, in_dim, num_classes, device, epochs=10, bs=2048, lr=1e-3):
    model = PatchClassifier(in_dim, num_classes).to(device)
    opt = torch.optim.Adam(model.parameters(), lr=lr)
    crit = nn.CrossEntropyLoss()

    tr_ds = torch.utils.data.TensorDataset(X_tr, y_tr)
    va_ds = torch.utils.data.TensorDataset(X_va, y_va)
    tr_loader = DataLoader(tr_ds, batch_size=bs, shuffle=True)
    va_loader = DataLoader(va_ds, batch_size=bs, shuffle=False)
...
def run_content_probe(model, val_loader, device, patch_size, probe_epochs=10, sample_frac=1.0):
    # 1. extract tokens, patches, labels
    tokens, patches, labels = extract_tokens_patches_labels(model, val_loader, device, patch_size)
    print("Extracted:", tokens.shape, patches.shape, labels.shape)

    # optional subsample to speed experiments
    if sample_frac < 1.0:
        sel = torch.randperm(tokens.shape[0])[:int(tokens.shape[0]*sample_frac)]
        tokens = tokens[sel]; patches = patches[sel]; labels = labels[sel]

    # flatten patches for decoder target
    B = patches.shape[0]
    C, pH, pW = patches.shape[1:]
    patch_flat = patches.view(B, -1)  # in [0,1] probably

    # split high vs normal tokens
    high_mask, normal_mask, thr, mu = split_high_normal(tokens)
    print(f"Threshold {thr:.4f} (mean={mu:.4f}). high_count={high_mask.sum().item()}, normal_count={normal_mask.sum().item()}")

    # build 80/20 splits for each group
    def splits_from_mask(mask):
    ...
    high_s = splits_from_mask(high_mask)
    norm_s = splits_from_mask(normal_mask)
    in_dim = tokens.shape[1]
    out_dim = patch_flat.shape[1]
    num_classes = int(labels.max().item()) + 1

    results = {}
    # HIGH group
    if high_s is not None:
        tr_idx, va_idx = high_s
        Xtr, Ytr = tokens[tr_idx].float(), patch_flat[tr_idx].float()
        Xva, Yva = tokens[va_idx].float(), patch_flat[va_idx].float()
        ...
        print("No high tokens found.")

    # NORMAL group
    if norm_s is not None:
        tr_idx, va_idx = norm_s
        Xtr, Ytr = tokens[tr_idx].float(), patch_flat[tr_idx].float()
        Xva, Yva = tokens[va_idx].float(), patch_flat[va_idx].float()
        Ytr_cls, Yva_cls = labels[tr_idx], labels[va_idx]
        ...
        print("No normal tokens found.")

    # print summary
    print("\n=== Content probing summary ===")
    for k,v in results.items():
        print(f"{k.upper():6s} | MSE: {v['mse']:.6f} | PSNR: {v['psnr']:.3f} dB | cls_acc: {v['acc']*100:.2f}%")
    print("================================")
    return results
...
if __name__ == "__main__":
    ...
    print("Running content probing on validation set...")
    content_results = run_content_probe(model, val_loader, device, patch_size=14, probe_epochs=10, sample_frac=1.0)
